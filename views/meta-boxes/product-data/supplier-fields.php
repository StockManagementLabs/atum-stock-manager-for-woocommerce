...
if ( empty( $variation ) ) : ?>
<div class="options_group <?php echo esc_attr( implode( ' ', $supplier_fields_classes ) ) ?>">
<?php endif; ?>
	...
	<p class="form-field _supplier_field<?php if ( ! empty( $variation ) ) echo ' form-row form-row-first' ?>">
		...
		<span class="atum-field input-group">
			...
			<select class="wc-product-search" id="<?php echo esc_attr( $supplier_field_id ) ?>" name="<?php echo esc_attr( $supplier_field_name ) ?>" style="width: <?php echo ( empty( $variation ) ) ? 80 : 100 ?>%"
				...
				data-multiple="false" data-selected="" data-minimum_input_length="1"<?php echo esc_attr( apply_filters( 'atum/views/meta_boxes/supplier_fields/supplier', '', $variation, $loop ) ) ?>
				>
			...
			<?php echo wc_help_tip( esc_attr__( 'Choose a supplier for this product.', ATUM_TEXT_DOMAIN ) ); // WPCS: XSS ok. ?>
		</span>
	</p>
	...
	<p class="form-field _supplier_sku_field<?php if ( ! empty( $variation ) ) echo ' form-row form-row-last' ?>">
		...
		<span class="atum-field input-group">
			...
			<input type="text" class="short" style="" name="<?php echo esc_attr( $supplier_sku_field_name ) ?>"
				...
				<?php echo esc_attr( apply_filters( 'atum/views/meta_boxes/supplier_fields/supplier_sku', '', $variation, $loop ) ) ?>
				>
