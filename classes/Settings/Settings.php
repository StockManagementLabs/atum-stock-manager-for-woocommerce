	...
	public function enqueue_scripts( $hook ) {
		...
		if ( in_array( $hook, [ Globals::ATUM_UI_HOOK . '_page_' . self::UI_SLUG, 'toplevel_page_' . self::UI_SLUG ] ) ) {
			...
			wp_localize_script( self::UI_SLUG, 'atumSettingsVars', array(
				...
				'outStockThresholdSetClearText'   => __( "We did save all your previous 'Out of stock' values the last time you used this option. Would you like to clear all the saved data and to start fresh? If you've added new products since then, these will just use the global WooCommerce value (if set).", ATUM_TEXT_DOMAIN ),
				'outStockThresholdDisable'        => __( "We are going to leave your saved values untouched in case you decide to re-enable the ATUM's Out of Stock threshold per product again. Please, click OK to start using the WooCommerce's global Out of Stock threshold value (if set).", ATUM_TEXT_DOMAIN ),
