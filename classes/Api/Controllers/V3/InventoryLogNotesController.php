...
class InventoryLogNotesController extends AtumOrderNotesController {
	...
	public function get_items( $request ) {
		...
		$order = new Log( (int) $request['order_id'] );

		if ( ! $order || $this->post_type !== $order->get_type() ) {
			return new \WP_Error( "atum_rest_{$this->post_type}_invalid_id", __( 'Invalid inventory log ID.', ATUM_TEXT_DOMAIN ), [ 'status' => 404 ] );
		}
	...
	public function get_item( $request ) {
		...
		if ( ! $order || $this->post_type !== $order->get_type() ) {
			return new \WP_Error( 'atum_rest_order_invalid_id', __( 'Invalid inventory log ID.', ATUM_TEXT_DOMAIN ), [ 'status' => 404 ] );
		}
	...
	/**
	 * Create a single inventory log note
	 *
	 * @since 1.6.2
	 *
	 * @param \WP_REST_Request $request Full details about the request.
	...
	/**
	 * Delete a single inventory log note
	 *
	 * @since 1.6.2
	 *
	 * @param \WP_REST_Request $request Full details about the request.
