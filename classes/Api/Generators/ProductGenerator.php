...
class ProductGenerator extends GeneratorBase {
	...
	protected function prepare_data( array $product ): array {
		...
		return array_merge( $base_fields, [
			...
			'price'                  => ! $this->is_null_value( $product['price'] ) ? (float) $product['price'] : NULL,
			'regularPrice'           => ! $this->is_null_value( $product['regular_price'] ) ? (float) $product['regular_price'] : NULL,
			'salePrice'              => ! $this->is_null_value( $product['sale_price'] ) ? (float) $product['sale_price'] : NULL,
			'purchasePrice'          => ! $this->is_null_value( $product['purchase_price'] ) ? (float) $product['purchase_price'] : NULL,
			...
			'stockQuantity'          => ! $this->is_null_value( $product['stock_quantity'] ) ? (int) $product['stock_quantity'] : NULL,
			'stock'                  => ! $this->is_null_value( $product['stock'] ) ? (int) $product['stock'] : NULL,
			...
			'inventoryStock'         => ! $this->is_null_value( $product['inventory_stock'] ) ? (int) $product['inventory_stock'] : NULL,
			'inventoryMainStock'     => ! $this->is_null_value( $product['inventory_main_stock'] ) ? (int) $product['inventory_main_stock'] : NULL,
